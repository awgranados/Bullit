...
import { Avatar, Button, Card, Accordion, List, Title, Paragraph, TouchableRipple } from 'react-native-paper';
                    ...
                    >
                        <View style={{ marginTop: 10, alignItems: 'center' }}>
                            {rideOffers.map((offer, index) => (
                                <TouchableRipple key={index} onPress={() => openDetailScreen(offer)}>
                                    <Card key={index} style={styles.card}>
                                        ...
                                        <Card.Content style={{flexDirection: 'row', alignItems: 'center'}}>
                                            <View style={{marginRight: 16}}>
                                                <Title style={{fontSize: 30, fontWeight: 'bold', color:"white"}}>{22}</Title>
                                                <Paragraph  style={{ textAlign: 'center', fontSize: 20, color:"white" }}>{"Nov"}</Paragraph>
                                            </View>
                                            <View style={styles.iconContainer}>
                                                <MaterialIcons name={"my-location"} size={25} color={"white"} marginTop={15}/>
                                                <Entypo name={"dots-three-vertical"} size={25} color={"white"} marginTop={5}/>
                                                <MaterialIcons name={"location-pin"} size={25} color={"white"} marginTop={3}/>
                                            </View>
                                            <View style={{ position: 'relative' }}>
                                                {/* if driver then using steering wheel icon, else use passenger icon */}
                                                {/* SUBSTITUTE */}
                                                {isDriver ? (
                                                    <MaterialCommunityIcons name={"steering"} size={50} color={"white"} marginTop={15} position={'absolute'} top={-70} right={-230} />
                                                ) : (
                                                    <MaterialCommunityIcons name={"seat-passenger"} size={50} color={"white"} marginTop={15} position={'absolute'} top={-70} right={-230} />
                                                )}
                                                <Text style={{color: "red", position: 'absolute', top: 10, right: -230}}>{"Reserved:"}</Text>
                                                {/* SUBSTITUTE */}
                                                <Text style={{color: "red", marginTop:20, position: 'absolute', top: 10, right: -226}}>{"1/6 seats"}</Text>
                                            </View>
                                            <View >
                                                <Text style={{fontSize:20, color:"white"}}>{"UCSB"}</Text>
                                                <Text style={{color:"white"}}>{"11:00 AM"}</Text>
                                                <Text style={{fontSize:20, marginTop:5, color:"white"}} >{offer.destination && offer.destination.split(',')[0]}</Text>
                                ...
                                <TouchableRipple key={index} onPress={() => openDetailScreen(offer)}>
                                    <Card key={index} style={styles.card}>
                                        ...
                                        <Card.Content style={{flexDirection: 'row', alignItems: 'center'}}>
                                            ...
                                            <View style={{ position: 'relative' }}>
                                                <Text style={{color: "white", position: 'absolute', marginTop:15, top: -70, right: -230}}>{"Total Earning:"}</Text>
                                                {/* SUBSTITUTE */}
                                                <Text style={{color: "white", position: 'absolute', marginTop:35, top: -70, right: -226}}>{"$60"}</Text>
                                                <Text style={{color: "red", position: 'absolute', top: 10, right: -230}}>{"Reserved:"}</Text>
                                                {/* SUBSTITUTE */}
                                                <Text style={{color: "red", marginTop:20, position: 'absolute', top: 10, right: -226}}>{"1/6 seats"}</Text>
                                            </View>
